...
class suratjalan extends CI_Controller {
	...
	function GetDataFaktur(){
			...
			$ResultFaktur = $this->db->query("SELECT BC.nameClient AS NamaClient,
											bcar.CarBrand AS MobilBrand,
											bcar.CarName AS NamaMobil,
											bf.Hari AS Hari
											FROM bahana_faktur bf 
											LEFT JOIN bahana_client BC ON bf.client=BC.id
											LEFT JOIN bahana_car bcar ON bf.DataMobil=bcar.id_car
			...
			if($data){
				echo json_encode(array('status' => 1, 'DataClient' => ''.$data[0]->NamaClient.'','DataMobil' =>''.$data[0]->NamaMobil.'','Hari' =>''.$data[0]->Hari.'' ))	;			
			}else{
	...
	function simpan()
	{
		...
		if($this->session->userdata("logged_in")=="yesGetMeLoginBaby")
		{
			...
			{
				//KALO APPROVED SURAT JALAN
				if(isset($_POST['NomorSJ'])){$UpdateDt['NomorSJ'] = $this->security->xss_clean($this->input->post('NomorSJ'));}
				if(isset($_POST['timeupdated'])){$UpdateDt['TimeUpdate'] = $this->security->xss_clean($this->input->post('timeupdated'));}
				if(isset($_POST['dateupdated'])){$UpdateDt['DateUpdate'] = $this->security->xss_clean($this->input->post('dateupdated'));}
				if(isset($_POST['JenisSimpan'])){$UpdateDt['status'] = $this->security->xss_clean($this->input->post('JenisSimpan'));}
				//if(isset($_POST['uangjalan'])){$UpdateDt['uangjalan'] = $this->security->xss_clean($this->input->post('uangjalan'));}
				$UpdateDt['update_by'] = $this->session->userdata['id'];
				//pr($ApproveDt);exit;

				$this->db->where('NomorSJ', $UpdateDt['NomorSJ']);
				$this->db->update("bahana_surat_jalan",$UpdateDt);
